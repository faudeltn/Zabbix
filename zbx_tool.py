...
class zabbix_api:
    def __init__(self):
        self.url = 'https://zabbix.yallalabs.com/api_jsonrpc.php'
    ...
    def user_login(self):
        ...
        try:
            ...
            print ("\033[041m Authentication failed, please check URL !\033[0m",e.code)
        except KeyError as e:
            print "\033[041m Authentication failed, please check username and password !\033[0m",e
        else:
    ...
    def hostgroup_id(self, hostgroupName=''):
        data = json.dumps({
                           "jsonrpc":"2.0",
                           "method":"hostgroup.get",
                           "params":{
    ...
    def hostgroup_list(self, hostgroupName=''):
        data = json.dumps({
                           "jsonrpc":"2.0",
                           "method":"host.get",
                           "params":{
                                     "output": "extend",
                                     "groupids": self.hostgroup_id(hostgroupName),
                                     },
                           "auth":self.user_login(),
                           "id":1,
                           })
        ...
        try:
            ...
            for group in response['result']:
                if  len(hostgroupName)==0:
                    print "hostName:  \033[31m%s\033[0m \thostid has: %s" %(group['name'],group['hostid'])
                else:
                    print "hostName:  \033[31m%s\033[0m Host: \033[31m%s \033[0m \thostid : \033[31m%s\033[0m " %(group['name'],group['host'],group['hostid'])
                    self.hostgroupID = group['hostid']
            return group['hostid']
...
if __name__ == "__main__":
    ...
    parser.add_argument('-L','--list',nargs='?',dest='listhostbygroup',default='list',help='查询主机组')
    ...
    if len(sys.argv) == 1:
        ...
        if args.listhostbygroup != 'list':
            if args.listhostbygroup:
                zabbix.hostgroup_list(args.listhostbygroup)
            else:
